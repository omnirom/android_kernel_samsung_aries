...
void set_otghost_mode(int mode) {
	...
	if (mode==-1)
		mode = atomic_read(&g_OtgLastCableState);
	...
	switch (opmode) {
		...
		case 'a':
			...
			break;
		default:
			atomic_set(&g_OtgOperationMode,'c');
			enable = 0;
			break;
	}
...
static ssize_t usbmode_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch(atomic_read(&g_OtgHostMode)) {
		case USB_OTG_DRIVER_S3CHS:
			msg3 = "host (S3C/HS)";
			break;
		case USB_OTG_DRIVER_S3CFSLS:
			msg3 = "host (S3C/FSLS)";
			break;
		case USB_OTG_DRIVER_DWC:
			msg3 = "host (DWC)";
			break;
	}
...
static ssize_t usbmode_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch(c) {
		case 'a':
			atomic_set(&g_OtgOperationMode, 'a');
			set_otghost_mode(-1);
			break;
		case 'h':
			atomic_set(&g_OtgOperationMode, 'h');
			set_otghost_mode(-1);
			break;
		case 'c':
			atomic_set(&g_OtgOperationMode, 'c');
			...
			break;
		case 'o':
			atomic_set(&g_OtgOperationMode, 'o');
			set_otghost_mode(-1);
			break;
		default:
...
static ssize_t usbdriver_read(struct device *dev, struct device_attribute *attr, char *buf) {
	...
	switch (atomic_read(&g_OtgDriver)) {
		case USB_OTG_DRIVER_S3CFSLS:
			msg = "l:S3C driver (low-speed / full-speed)";
			break;
		case USB_OTG_DRIVER_DWC:
			msg = "d:DWC driver";
			break;
...
static ssize_t usbdriver_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t size) {
	...
	switch (c) {
		...
		case 'h':
			atomic_set(&g_OtgDriver, USB_OTG_DRIVER_S3CHS);
			break;
		case 'l':
			atomic_set(&g_OtgDriver, USB_OTG_DRIVER_S3CFSLS);
			break;
#endif
